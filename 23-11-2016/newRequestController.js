...
'smartTable'], function (app, RequestsModule, smartTable) {
    ...
    RequestsModule.controller('newRequestController', function ($scope, $rootScope, $state, $filter, $q, $timeout, $window, $mdDialog, newRequestService, ARPService, formSteps, userAccessDetailsFactory, AllReferenceDataService, Restangular, RefRestangular, StorageService, STORAGE_TYPES, formlyConfig, formlyValidationMessages, formFactory, FORM_TEMPLATE, VIEW_PATH, _requestableType, _currentUser, BriefcaseService, SHOW_ASSIGN_MODEL, SCROLLBAR_DIV_HEIGHT, PRECANNED_POLICY, PRECANNED_FIELDS, LIMITS, INCLUDE, REQUEST_STATUS, ACCESS_DISPLAY_CHAR, END_DATE_POLICY, PAGINATION_COUNT, referenceDataFactory) {
    	...
    	formlyConfig.extras.errorExistsAndShouldBeVisibleExpression ='!fc.$pending';
    	 formlyValidationMessages.messages.required = 'name + " is required"'
        //formlyValidationMessages.addStringMessage('required', name + " is required"');
        ...
        $scope.sync = function (bool, item) {
           ...
           // } else {
        	$scope.data =$filter('filter')($scope.optionalApplicationsArr,{'isSelected' :true},true)
        ...
        $scope.disableDepApps = function (applicationLabel) {
            if (!angular.isUndefinedOrNull($scope.optionalApplications[applicationLabel].addDependencies) && $scope.optionalApplications[applicationLabel].addDependencies != " ") {
        ...
        $scope.pushRequired = function (metadata) {
        	console.log("calling $scope.pushRequired function");
            var arrRequired = {
            };
            var i = 1;
        ...
        $scope.validateForm = function ($event) {
            ...
            if (angular.isDefined($scope.arrRequired)) {
                ...
                angular.forEach($scope.arrRequired, function (keyDetails, key) {
                    ...
                    switch (keyType) {
                        case 'multicheckbox':
                        	console.log("validation start for multicheckbox");
                        	console.log("!angular.isUndefinedOrNull($scope.vm.model[key])",!angular.isUndefinedOrNull($scope.vm.model[key]));
                        	console.log("angular.equals($scope.frmData.application, 'RPM')",angular.equals($scope.frmData.application, 'RPM'));
                        	console.log("key",key);
                        	console.log("$scope.vm.model[key]",$scope.vm.model[key]);
                        	
                            if (!angular.isUndefinedOrNull($scope.vm.model[key]) && angular.equals($scope.frmData.application, 'RPM')) {
            ...
            if ($scope.isValidated()) {
            	// not able to understand what below function is doing
        ...
        $scope.checkAndValidateForms = function ($event) {
        	...
        	//	$scope.frmData.application = application.name;
        		 switch (application.name) {
                 case 'analyticalRoles':
                	 isValid = $scope.validateAnalyticalForm($event);
                     break;
                 default:
                	 isValid = $scope.validateForm($event);
             }
        ...
        $scope.isPreCanned = function () {
        	console.log("$scope.frmData.accessType",$scope.frmData.accessType);
        ...
        $scope.openDialog = function ($event) {
            if ($scope.isPreCanned()) {
            	...
            	//console.log("entering into next tab", $scope);
            	// not able to understand the logic so commented the function
                //$scope.nextTab();
            }
        };
        ...
        $scope.vm.onSubmit = function ($event) {
            ...
            $timeout(function () {
               ...
               // customValidation = $scope.checkAndValidateForms($event);
                ...
                console.log("$scope.vm.form.$valid",$scope.vm.form.$valid);
        ...
        $scope.changeBeneficiary = function (index) {
            console.log('changeBeneficiary function called!');
            $scope.frmData = {
            };
        ...
        $scope.startOver = function () {
            console.log('startOver function called!');
        ...
        $scope.reloadState = function (resetBeneficiary) {
            console.log('reloadState function called!');
            $scope.showNoChange = false;
            $scope.enterInside = true;
            $scope.frmData = {
            };
        ...
        $scope.prepareFormFields = function (resData, resDateData) {
        	console.log("prepareFormFields called");
            var formFields = {
            };
        ...
        $scope.getMetaDataFromLocal = function () {
            ...
            var uniqueApplicationList = new Array();
            var isPromiseExist = false;
            angular.forEach($scope.frmData.applicationList, function (application) {
            	if(uniqueApplicationList.length>0){
            	 isPromiseExist = $filter('filter')(uniqueApplicationList,{"name":application.name},true).length>0;
            	}
            	if(!isPromiseExist && angular.isDefined(application.hasApplicationAttributes) && application.hasApplicationAttributes.toLowerCase() == 'yes' && application.name != 'RG_DA' && application.name !='RG_MR' ){
            		uniqueApplicationList.push(application);
            		console.log("application.hasApplicationAttributes : "+application.hasApplicationAttributes+" applicationName : "+application.name);
                ...
                angular.forEach(response, function (data) {
                    angular.forEach(data.data[Object.keys(data.data) [0]], function (element) {
                        if (element.key) {
                            ...
                            element.data.appType = $filter('filter')($scope.frmData.applicationList,{"name":element.data.applicationName},true)[0].appType;
                        }
                        metadata.push(element);
                    })
                })
                
                $scope.arrRequired = $scope.pushRequired(response);
        ...
        $scope.$watch('frmData.application', function (current, old) {
            if (current !== old) {
                $scope.tabLocked.thirdLocked = true;
                $scope.tabLocked.fourthLocked = true;
                setTimeout(function () {
                    var tabItem = document.getElementsByTagName('md-tab-item');
                    for (var i = 2; i < tabItem.length; i++) {
        ...
        $scope.requestSelf = function (event) {
            ...
            $scope.getUserDetails($scope.loggedUser.userId).then(function (res) {
                ...
                $scope.beneficiaryDisplay = $scope.profile.displayName; 
                $scope.beneficiaryUserId = $scope.profile.id; 
                formSteps.loadStandardProfile($scope);  // Load role/department/region again
            },            // error
            function (response) {
            }
            );
        ...
        $scope.selectReportess = function (reporteesDetails) {
            ...
            $scope.beneficiaryUserId = reporteesDetails.id;
            ...
            $scope.getUserDetails($scope.frmData.beneficiary).then(function (res) {
                if (res.status == 404) {
                    ...
                    $scope.beneficiaryUserNameDisplay = $scope.profile.displayName;
                    formSteps.loadStandardProfile($scope); // Load role/department/region again
