define(['app', 'angularAMD', 'smartTable'], function (app, RequestsModule, smartTable) {
    /*
     * Navigate the list and select the user using keybaord
     * DO NOT MOVE this arrowselector directive 
     */
    RequestsModule.directive('arrowSelector', ['$document', function ($document) {
            return{
                restrict: 'A',
                ...
                $scope.validateAccess = function (valid) {
                    ...
                    angular.forEach($scope.dataAccess, function (value, key) {
                        if (keepGoing) {
                            ...
                            if (!angular.equals(value, {}) && angular.equals(value, oldValue)) {
                                keepGoing = false;
...
//Load the questions for the application CORE  
                ...
                $scope.loadCoreQuestions = function () {
                    ...
                    referenceDataFactory.getData($scope.addParams, "snp.refData.CORE.questions.data")
...
//Resize tree dropdown width as per viewport change
                ...
                $scope.$watchGroup(['navMenuState', 'layoutArea'], resizeTreeDropdown);
...
//Find all the custom mandatory field from metadata 
                $scope.pushRequired = function (metadata) {
                    ...
                    angular.forEach(metadata, function (dataElement) {
                        var multiselect = {};
                        if (angular.isDefined(dataElement.fieldGroup) && $filter('filter')(dataElement.fieldGroup, {"type": "multiselect"}, true).length) {
                            multiselect = $filter('filter')(dataElement.fieldGroup, {"type": "multiselect"}, true)[0];
                        }

                        if (!angular.equals(multiselect, {}) && angular.equals(multiselect.type, 'multiselect')) {
...
//Validate the forms other than Analytical              
                $scope.validateForm = function ($event) {
                    ...
                    if (angular.isDefined($scope.arrRequired)) {
                            ...
                            switch (keyType) {
                                case "multiselect":
                                    ...
                                    angular.forEach($scope.vm.fields, function (fieldElement) {
                                        if (!fieldFound && angular.isDefined(fieldElement.fieldGroup) && $filter('filter')(fieldElement.fieldGroup, {"key": key}, true).length) {
                                            //console.log('fieldElement',fieldElement);
                                            var multiselectField = $filter('filter')(fieldElement.fieldGroup, {"key": key}, true)[0];
                        ...
                        if ($scope.frmData.application == 'keyRatingsApplications') {
...
// Open dialog box on click of next in form step //
                $scope.openDialog = function ($event) {
                    if ($scope.isPreCanned()) {
                        $scope.convertToPrecannedFormat().then(
                                function (res) {
                                    var scope = res;
                                    //&& scope.length
...
//Create the complete string to display Roles
                    ...
                    angular.forEach($scope.vm.model['roleRequestables'], function (element, key) {
                        if (element) {
                            var roleLabel = $filter('filter')(multicheckboxData, {"name": key}, true)[0];
...
// Precanned Bucket validatation with different criteria
                $scope.precannedBucketValidate = function (response) {
                    ...
                    } else if (Object.keys(response.children).length == 2 && $filter('filter')(response.children, {"name": "multipleDepartments"}).length) {
                        var arrFilter = $filter('filter')(response.children, {"name": "!multipleDepartments"});
...
//                }
                ...
                $scope.vm.onSubmit = function () {
                    ...
                    $rootScope.$broadcast('onSubmit', {"submit": false});
                    ...
                    $timeout(function () {
                        ...
                        if ($scope.vm.form.$valid && customValidation) {
                            $scope.nextTab();
                        }
                    }, 1000)
...
//Reset Selected user's access data if not coming from Access page
                ...
                if ($scope.previousState != 'access.user' &&
                        $scope.previousState != 'access.current' &&
                        $scope.previousState != 'inbox.request') {
                    userAccessDetailsFactory.reset();
                }
...
//Redirect the user to Access page if user press cancle in form stage
                $scope.cancelUserAccess = function () {
                    ...
                    var params = {};
                    if ($scope.requestId) {
                        params = {requestId: $scope.requestId};
                    }
                    ...
                    $scope.getUserDetails($scope.frmData.beneficiary).then(
                            function (res) {
                                ...
                                $scope.setPolicy().then(
                                    ...
                                    $scope.$watchGroup(['requestablePromise', 'userRequestModeAccountPromise'], function (newVal, oldVal) {
                                        if (newVal[0] === true && newVal[1] === true) {
                                            var requestableArray = $scope.requestableName[$scope.frmData.accessKey];
                                            var filterobj = {'endpoint': $scope.userAccessData.accessTarget};
...
//Check If PATCH has no change //
                            ...
                            var filterWithoutCrossPractice = $filter('filter')(brifcaseData.reqdata.Operations[0].data.Operations, {"path": "!crossPractice"});
...
//$watch new request steps 
                        $scope.$watch('selectedIndex', function (current, old) {                            
                            ...
                            if (current != 0) {
                                if (angular.isObjectEmpty($scope.userAccessData) ||
                                        (!angular.isObjectEmpty($scope.userAccessData) && current > 2)) {
                                    $(".md-tab").eq(old).addClass("md-visited");
                                }
...
// Get selected access Type Information from requestable
                        $scope.selectedAccessInfo = function () {
                            if ($filter('filter')($scope.requestableName[$scope.frmData.accessKey], {endpoint: $scope.frmData.application}, true).length) {
                                return $filter('filter')($scope.requestableName[$scope.frmData.accessKey], {endpoint: $scope.frmData.application}, true)[0];
                            }
                        }
                        ...
                        $scope.showPostRevoke = function () {
                            ...
                            if (angular.isDefined(objSearched.attributes) && angular.isDefined(objSearched.attributes.postConditionRevoke)) {
                                ...
                                angular.forEach(postConditionRevoke, function (accessType, key) {
                                    ...
                                    if ($filter('filter')($scope.userRequestModeAccount, {'target_name-value': accessType}, true).length) {
                                        var filterAccount = ($filter('filter')($scope.userRequestModeAccount, {'target_name-value': accessType}, true)[0]);
...
//Loads the revised data from API
                        $scope.patchformModel = function () {
                            ...
                            if ($scope.enterInside) {
                                ...
                                var postDetailsPromise = Restangular.one("users", $scope.modeRequestId).get({'midas-target': $scope.frmData.application});
...
//Loads the data from API for fields that are configured in treedropdown meta-data 
                        $scope.getTreedropdownDataFromAPI = function (addParams, refData, key) {
                            var deferred = $q.defer();
                            referenceDataFactory.getData(addParams, refData)
                                    .then(function (response) {
                                        var objResponse = {
                                            key: key,
...
//Put the same data in the scope. So that it will be available before the form load                       
                        $scope.loadTreedropdown = function (filterTreedropdown) {
                            ...
                            var filterByKey = $filter('filter')(treedropdownJson.templateOptions.fields[0].fieldGroup, {"type": "treedropdown"}, true);
...
// Common function for End date forms & General forms
                        $scope.prepareFormFields = function (resData, resDateData) {
                            ...
                            var filterWithoutJustification = $filter('filter')(resData.elements, {"name": "!Justification"});
                            var filterJustification = $filter('filter')(resData.elements, {"name": "Justification"}, true);
                            ...
                            if ($scope.isPreCanned()) {
                                formFields = $filter('filter')(resData.elements, {"name": "!Justification"});
                                var filterJustification = $filter('filter')(resData.elements, {"name": "Justification"}, true);
                                angular.forEach(filterJustification, function (justificationElement) {
                                ...
                                var filterJustification = $filter('filter')(formFields, {"info": "Justification"}, true);
...
//Watch added on vm.model.justification and customPrecannedJustification to truncate the fields value if its exceed the length
                                $scope.$watchGroup(['customPrecannedJustification', 'vm.model.justification'], function (current, old) {
...
//Its called when the user change the application on access step                        
                                $scope.changeapplication = function () {
                                    ...
                                    if (angular.isDefined($scope.selectedObj.prerequisite)
                                            && !angular.equals($scope.selectedObj.prerequisite, "")) {
...
// Process requestable Response //
                                $scope.processRequestable = function (requestableData, showUnauthAccess) {
                                    ...
                                    if (localStorage.getItem("inboxBulk") != null) {
                                        ...
                                        var searchObj = {'beneficiaryId': $scope.frmData.beneficiary.toUpperCase()};
